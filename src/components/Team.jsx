...
Meet the minds behind our success — a team of passionate experts whose unique skills help drive real results for our clients.              </p>
            </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16">
          {teamMembers.map((member, index) => (
            <a
              key={member.id}
              href={member.link}
              className="flex flex-col text-center no-underline group cursor-pointer transform transition-all duration-300 hover:-translate-y-1"
              style={{ 
                color: '#525866',
            ...
            >
              ...
              <div className="relative mb-6 overflow-hidden text-textColor">
                <img
                  src={member.image}
                  alt={member.name}
                  className="w-full h-[26rem] object-cover rounded-[24px] transition-all duration-500 group-hover:scale-105"
                  style={{
                    filter: 'blur(0px)',
