        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="text-center mb-12 md:mb-16">
            ...
            <div className="max-w-2xl mx-auto py-4">
              <h2 className="text-4xl md:text-5xl lg:text-[48px] font-medium text-textColor mb-4 leading-wider tracking-tight">
          ...
          <div className="grid gap-8 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
            {plans.map((plan, i) => (
              <motion.div
                key={i}
                className={`${plan.bg} rounded-2xl p-6 sm:p-8 flex flex-col relative`}
              ...
              >
                ...
                <div className="flex-grow">
                  <h3 className={`text-2xl font-medium mb-2 ${plan.text}`}>{plan.title}</h3>
                  <div className={`text-4xl sm:text-5xl text-center font-semibold mb-4 ${plan.text}`}>{plan.price}</div>
                  <p className={`text-sm sm:text-base mb-6 ${plan.text === 'text-white' ? 'text-blue-100' : 'text-textColor'}`}>
                    {plan.description}
                  </p>
                ...
                <Link
                  to="/contact-us"
                  className="mt-6  w-full bg-secondary text-textColor text-center py-3 rounded-full font-medium transition hover:bg-hoverColor inline-flex items-center justify-center gap-2"
                  onClick={() => {
                    window.scrollTo({ top: 0, behavior: 'smooth' });
                  }}
                >
                  Get Started
                  <img src={navArrow} alt="arrow" className="w-4 h-4" />
                </Link>
